...
if ($method === 'GET') {
    ...
    if (!empty($_GET['id'])) {
        $stmt = $pdo->prepare('SELECT id, username, role, is_active, access_expires_at, created_at, updated_at FROM `users` WHERE id = :id LIMIT 1');
    ...
    $stmt = $pdo->query('SELECT id, username, role, is_active, access_expires_at, created_at, updated_at FROM `users` ORDER BY id ASC');
    $users = $stmt->fetchAll();
    ...
    json_response(['success' => true, 'count' => count($users), 'users' => $users]);
}
...
if ($method === 'PUT') {
    ...
    // Fetch updated user
    $refetchStmt = $pdo->prepare('SELECT id, username, role, is_active, access_expires_at, created_at, updated_at FROM `users` WHERE id = :id LIMIT 1');
