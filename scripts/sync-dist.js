...
// 4. Synchronize api/ backend into dist/api and public_html/api (strictly excluding config.php)
function copyApiDirExcludingConfig(srcDir, destDir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      copyApiDirExcludingConfig(srcPath, destPath);
    } else {
      // Completely EXCLUDE config.php from being copied or overwritten anywhere
      if (entry.name === 'config.php') {
        console.log(`🔒 Excluded ${entry.name}: never copied or overwritten in ${path.relative(projectRoot, destPath)}`);
        continue;
      }
...
if (fs.existsSync(sourceApiDir)) {
  ...
  copyApiDirExcludingConfig(sourceApiDir, distApi);
  copyApiDirExcludingConfig(sourceApiDir, pubApi);
  console.log('✅ Synchronized api/ backend into dist/api and public_html/api (config.php strictly excluded)');
}
...
if (fs.existsSync(publicDir)) {
  ...
  for (const file of files) {
    if (file.startsWith('~$') || file === '.htaccess' || file === 'config.php') continue;
